...
smartctl_version="$(/usr/local/sbin/smartctl -V | head -n1 | awk '$1 == "smartctl" {print $2}')"
...
device_list="$(/usr/local/sbin/smartctl --scan-open | awk '/^\/dev/{print $1 "|" $3}')"
...
for device in ${device_list}; do
  ...
  /usr/local/sbin/smartctl -n standby -d "${type}" "${disk}" > /dev/null || active=0
  echo "device_active{disk=\"${disk}\",type=\"${type}\"}" "${active}"
  # Skip further metrics to prevent the disk from spinning up
  test ${active} -eq 0 && continue
  # Get the SMART information and health
  /usr/local/sbin/smartctl -i -H -d "${type}" "${disk}" | parse_smartctl_info "${disk}" "${type}"
  # Get the SMART attributes
  case ${type} in
  sat) /usr/local/sbin/smartctl -A -d "${type}" "${disk}" | parse_smartctl_attributes "${disk}" "${type}" ;;
